...
function redraw(givenstate, pos) {
  ...
  if (victory) {
	...
	var successColor = 'hsl(180,100%, 30%)'
	$('.finished').css('color', successColor);
	$('.timer').css('color', successColor);
	$('.puzzlename').css('color', successColor);
	$('.sudoku-cell').css('background-color', 'hsla(180,100%, 30%, .4)');
  } else {
    $('#victory').css('display', 'none');
	$('.timer').text(formatelapsed(state.elapsed));
	$('.finished').css('color', '');
	$('.timer').css('color', '');
	$('.puzzlename').css('color', '');
	$('.sudoku-cell').css('background-color', '').css('opacity', 1);
  }
  // Render timer UI
...
function highlightErrors(){
	var board = boardsofar(currentstate());
	conflicts = SudokuHint.conflicts(board);
	var errors = [];
	if(conflicts.length > 0){
		if(conflicts[0].errors.length > 0){ 
			$(document).trigger('log', ['placement_error', {count: conflicts[0].errors.length}]);
		}
		errors = conflicts[0].errors;
	}
	board.forEach(function(entry,id){
		var tile = $('#sc' + id);
		if( errors.indexOf(+id) > -1){
			tile.toggleClass('error', true);
		}else{
			tile.toggleClass('error', false);
		}
	});
}
...
$(document).on('mouseenter', 'td.sudoku-cell', function(ev) {
  ...
  $(this).toggleClass('cellFocus', true);
...
$(document).on('mouseleave', 'td.sudoku-cell', function(ev) {
  ...
  $(this).toggleClass('cellFocus', false);
...
$(document).on('contextmenu', 'td.sudoku-cell', function(ev){ev.preventDefault();
  //erase an entry on a right-click
  hidepopups();
  var pos = parseInt($(this).attr('id').substr(2));
  var state = currentstate();
  var validClear = (state.puzzle[pos] == null);
  $(document).trigger('log', ['leftclick_clear', {valid:  validClear, time: state.elapsed}]);
  // Ignore the click if the square is given in the puzzle.
  if (!validClear) return;
...
$(document).on('mousewheel DOMMouseScroll', function(ev){
	if(ev.originalEvent.detail > 0 || ev.originalEvent.wheelDelta > 0){//wheel up
		$(document).trigger('log', ['wheel', {direction: 1}]);
		if(curnumber == 0){
			setcurnumber(4);
		}else{
...
$(document).on('keydown', function(ev){
	//lets you toggle the number with the arrow keys
	if(ev.which == 38){//up
		if(curnumber == 0){
			setcurnumber(4);
		}else{
